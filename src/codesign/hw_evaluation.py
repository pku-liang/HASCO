    ...
    if tag in PARAM_TABLE:
        ...
        if acc.stt_matrix is None:
            config_file = gen_config(
                model_config_dir, parameterization, generator.type, tag)
        ...
        if acc == None:
            ...
            mapping_file = mapping_dir + generator.type + \
                "_" + benchmark.name + "_" + tag + ".m"
            l, t, p, _, a = model.evaluate(
                benchmark, schedules, acc, mapping_file)

            if l == model.max_val:
                PARAM_TABLE[tag] = None
                logger.error("Cannot find valid schedules.", exc_info=True)

            rst = {"latency": (l, 0.0), "throughput": (
                t, 0.0), "power": (p, 0.0), "area": (a, 0.0)}

            PARAM_TABLE[tag] = (rst, list(zip(schedules, all_tensors)))

        '''clean up'''
        if verbose == False and config_file is not None:
